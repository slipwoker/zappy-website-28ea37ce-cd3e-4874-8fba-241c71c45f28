document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getFieldError(field) {
      ...
      const label = (contactForm.querySelector('label[for="' + field.id + '"]') || {}).textContent || name;
      ...
      if (field.hasAttribute('minlength')) {
        ...
        if (!validators.minLength(value, min)) {
          return cleanLabel + ' must be at least ' + min + ' characters.';
        }
      }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '28ea37ce-cd3e-4874-8fba-241c71c45f28',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '28ea37ce-cd3e-4874-8fba-241c71c45f28';
      ...
      try {
        ...
        if (data.data.growSdkMode && data.data.authCode) {
          ...
          if (paymentSection && sdkContainer) {
            ...
            // Mutable slot updated on every checkout attempt so init callbacks
            // always redirect to the CURRENT order, not a stale first-attempt one.
            ...
            var growSdkEvents = {
              onSuccess: function() {
                ...
                var dest = cur.successUrl || (window.location.pathname.replace(/checkout.*/, '') + 'order-success?ref=' + encodeURIComponent(cur.reference || ''));
          ...
          // DOM containers missing — cannot render in-page SDK
...
(function () {
  const faqItems = document.querySelectorAll('.roni-faq-item');

  faqItems.forEach(function (item) {
    const btn = item.querySelector('.roni-faq-question');
    const answer = item.querySelector('.roni-faq-answer');

    btn.addEventListener('click', function () {
      const isActive = item.classList.contains('roni-active');

      // Close all open items
      faqItems.forEach(function (otherItem) {
        otherItem.classList.remove('roni-active');
        otherItem.querySelector('.roni-faq-question').setAttribute('aria-expanded', 'false');
        const otherAnswer = otherItem.querySelector('.roni-faq-answer');
        otherAnswer.setAttribute('hidden', '');
      });

      // Toggle current item
      if (!isActive) {
        item.classList.add('roni-active');
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '28ea37ce-cd3e-4874-8fba-241c71c45f28';
